...
vec3 getNormal(mat4 shadowMatrix, sampler2D textureNormal, vec3 position) {
	vec4 vShadowCoord = shadowMatrix * vec4(position, 1.0);
	vec4 shadowCoord  = vShadowCoord / vShadowCoord.w;
	vec2 uv = shadowCoord.xy;
...
void main(void) {
	...
	float posOffset = mix( extra.r, 1.0, 0.4) * 0.5;
	vec3 acc        = curlNoise(pos * posOffset + time * 0.3);
	
	vel += acc * .0005 * mix(extra.z, 1.0, .5);

	float dist = length(pos);
	if(dist > maxRadius) {
		float f = (dist - maxRadius) * .0005;
		vel -= normalize(pos) * f;
	}

	const float decrease = .976;
	vel *= decrease;

	pos += vel;

	float d0       = getDistToCamera(uShadowMatrix0, texture0, pos, uProjInvert0, uViewInvert0);
	float d1       = getDistToCamera(uShadowMatrix1, texture1, pos, uProjInvert1, uViewInvert1);
	
	float dd0      = abs(pos.z - d0);
	float dd1      = abs(pos.z - d1);
	pos.z    	   = dd0 < dd1 ? d0 : d1;

	vec3 normal0   = getNormal(uShadowMatrix0, textureNormal0, pos);
	vec3 normal1   = getNormal(uShadowMatrix1, textureNormal1, pos);

	vec3 N = dd0 < dd1 ? normal0 : normal1;

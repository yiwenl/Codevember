...
void main() {
	...
	float uvScale 		= 2.0;
	vec2 uv 			= vTextureCoord * vec2(uvScale, uvScale * PI);
	vec3 diffuseColor 	= texture2D(textureDiffuse, uv).rgb * uBaseColor;
	float specular 		= texture2D(textureSpecular, uv).r;

	vec3 nOffset 		= texture2D(textureNormal, uv).rgb * 2.0 - 1.0;
	vec3 N 				= normalize( vWsNormal + nOffset );
	vec3 V 				= normalize( vEyePosition );
	
	vec3 color 			= getPbr(N, V, diffuseColor, uRoughness, uMetallic, specular);

...
class ViewNight extends alfrid.View {
	
	constructor() {
		super(vs, fs);

		this.waveFront = new alfrid.EaseNumber(-1, 0.02);
		this.waveLength = 1;
	}


	_init() {
		this.mesh = Assets.get('houseNight');
		this.texture = Assets.get('colorNight');
		addCenter(this.mesh);
	}
	...
	render(mHit) {
		...
		this.shader.uniform("uInvert", "float", 1.0);
		GL.draw(this.mesh);
	}

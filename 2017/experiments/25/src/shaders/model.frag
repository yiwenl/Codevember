...
vec3 rotate(vec3 v) {
	float dist = distance(vPosOffset, uHit);
	dist = smoothstep(uRadius*0.5, uRadius, dist);
	float angle = vRotation.w + mix(vExtra.x, 1.0, .5) * uTime;
	angle = mod(angle, PI * 2.0) * uOffset * dist;
	return rotate(v, vRotation.xyz, angle);
}
...
void main(void) {
	...
	if(vPosition.x > vPosOffset.x + uSize.x ) {
		t = 0.0;
	} else if(vPosition.x < vPosOffset.x - uSize.x ) {
		t = 0.0;
	}

	if(vPosition.y > vPosOffset.y + uSize.y ) {
		t = 0.0;
	} else if(vPosition.y < vPosOffset.y - uSize.y ) {
		t = 0.0;
	}

	if(vPosition.z > vPosOffset.z + uSize.z ) {
		t = 0.0;
	} else if(vPosition.z < vPosOffset.z - uSize.z ) {
		t = 0.0;
	}

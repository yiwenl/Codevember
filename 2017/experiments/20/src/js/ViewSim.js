...
class ViewSim extends alfrid.View {
	...
	_init() {
		...
		this.shader.bind();
		this.shader.uniform('textureVel', 'uniform1i', 2);
		this.shader.uniform('texturePos', 'uniform1i', 3);
		this.shader.uniform('textureExtra', 'uniform1i', 4);
		this.shader.uniform('textureLife', 'uniform1i', 5);
		this.shader.uniform('textureOrg', 'uniform1i', 6);

	}


	render(textureVel, texturePos, textureExtra, textureLife, textureOrg, fbo0, fbo1, mShadowMatrix0, mShadowMatrix1, mProjInver0, mProjInver1, mViewInvert0, mViewInvert1) {
		this.time += .01;
		this.shader.bind();
		...
		this.shader.uniform("texture0", "uniform1i", 7);
		fbo0.getTexture().bind(7);
		this.shader.uniform("texture1", "uniform1i", 8);
		fbo1.getTexture().bind(8);

...
const vec3 windForce = vec3(-1.0, 0.5, -0.73) * 0.5;
...
void main(void) {
	...
	vec3 orgPos      = texture2D(textureLife, vTextureCoord).rgb;

	vec2 uvScreen	= getScreenUV(pos);
	float map 		= texture2D(textureMap, uvScreen).r;
	float invertMap = 1.0 - map;

	float posOffset = mix(extra.r, 1.0, .5) * .2;
